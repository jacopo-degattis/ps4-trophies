...
int sys_mknod(const char *path, mode_t mode, dev_t dev) {
  ...
  asm volatile(
      ...
      "mov rax, 14;"   // Moves the value 14 into the RAX register (which
                       // typically holds the system call number)
      "mov r10, rcx;"  // Moves the value of the RCX register into the R10
                       // register
      "syscall;" // Executes a system call using the values in the registers
      : "=a"(result), // Output constraint: Tells the compiler that the result
                      // of the operation will be stored in the RAX register
        "=@ccc"(err) // Output constraint: Indicates that error information will
...
int init_devices(void) {
  ...
  if (stat("/dev/pfsctldev", &s) == -1) {
    logger("BREW", "err stat pfsctldev");
    return -2;
  } else {
    if (sys_mknod("/dev/pfsctldev", S_IFCHR | 0777, s.st_dev) == -1) {
      logger("BREW", "err mknod pfsctldev");
      return -2;
    }
  }
  ...
  if (stat("/dev/lvdctl", &s) == -1) {
    logger("BREW", "err stat lvdctl");
    return -3;
  } else {
    if (sys_mknod("/dev/lvdctl", S_IFCHR | 0777, s.st_dev) == -1) {
      logger("BREW", "err mknod lvdctl");
      return -3;
    }
  }
  ...
  if (stat("/dev/sbl_srv", &s) == -1) {
    logger("BREW", "err stat sbl_srv");
    return -4;
  } else {
    if (sys_mknod("/dev/sbl_srv", S_IFCHR | 0777, s.st_dev) == -1) {
      logger("BREW", "err mknod sbl_srv");
      return -4;
    }
  }
...
int initVshDataMount(void) {
  logger("BREW", "Loading libSceFsInternalForVsh.sprx...");

  // initialize cred
  if (init_cred() != 0) {
    logger("BREW", "Failed to init cred");
    return (0);
  }

  // load private libraries, do once after setting cred
  if (loadPrivLibs() != 0) {
    logger("BREW", "Failed to load priv libs");
    return (0);
  }

  // create devices
  if (init_devices() != 0) {
    logger("BREW", "Failed to create devices");
    return (0);
  }

  /*if (setup_cred() != 0) {
      logger("BREW", 0, "Failed to setup cred\n");
      return(0);
  }*/

  logger("BREW", "VSH data mount initialized!");
...
int get_firmware_version(void) {
  ...
  if (sysKernelGetUpdVersion(&fw) && sysKernelGetLowerLimitUpdVersion(&fw)) {
    logger("BREW", "Error: can't detect firmware version!");
    return (-1);
  }

  logger("BREW", "[i] PS4 Firmware %d", fw >> 16);
  return (fw >> 16);
}
...
const char *get_fw_by_pfskey_ver(int key_ver) {
  const char *fw[] = {"???",   "Any",   "4.50+", "4.70+", "5.00+", "5.50+",
...
int initialize_jbc(void) {
  ...
  if (!jailbreak()) {
    logger("BREW", "Jailbreak failed!");
    Notify("Jailbreak failed!");
    return 0;
  }

  logger("BREW", "Jailbreak succeeded!");
  return 1;
}
